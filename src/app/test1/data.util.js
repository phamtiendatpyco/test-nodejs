...
const resoveldData = (data) => {
    ...
    return Object.keys(data)
        .map((k, currentIndex) => `${k}${EQUAL_SEPARATOR}${data[k]}`)
        .join(SEMI_COLON_SEPARATOR);
};
exports.storeData = (data) =>
    data.reduce(
        (result, currentData, currentIndex) =>
            `${result}${resoveldData(currentData)}${
                isNotLastElement(currentIndex, data.length)
                    ? LINE_SEPARATOR
                    : EMPTY_STRING
            }`,
        EMPTY_STRING
...
exports.loadData = (data) => {
    ...
    mapText.forEach((element) => {
        ...
        keyValueTexts.forEach((element1) => {
            ...
            resultKeyValueTexts[keyValueArray[KEY_INDEX]] =
                keyValueArray[VALUE_INDEX];
        });
