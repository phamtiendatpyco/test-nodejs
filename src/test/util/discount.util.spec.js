...
import discountUtil from '../../app/util/discount.util';
...
describe('DiscountUtil', () => {
  ...
  describe('getPercentageDiscountPrice', () => {
    it('should employeePercentageDiscountPrice successfully', async () => {
      ...
      // When
      const percentageDiscountPrice = await discountUtil.getPercentageDiscountPrice(user, product);
  ...
  describe('getPercentageDiscountPrice', () => {
    it('should loyalPercentageDiscountPrice successfully', async () => {
      // Given
      const user = new User("test", moment('20150101', 'YYYYMMDD'), false, false);
      const product = new Product(bignumber(100), ProductType.OTHERS);
  ...
  describe('getPercentageDiscountPrice', () => {
    it('should throw error when price is negative', async () => {
      // When
    const negativeNumber = () => discountUtil.isNegativeNumber(bignumber(-1)) ;

    // Then
    expect(negativeNumber).throw(TypeError);
    });
  });
